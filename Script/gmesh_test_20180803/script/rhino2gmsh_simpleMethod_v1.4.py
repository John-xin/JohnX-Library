...
def load2GeoDict(layerName):
    objs=rs.ObjectsByLayer(layerName,False) #return polysurfaces id list
        ...
        for surface in surfaces:
            ...
            for line in lines:
                ...
                for pt in pts: #ignore the last point as it is same as the first point
                    countPt+=1
                    ptDict.append({'ptIndex':countPt,'ptX':round(pt[0],6),'ptY':round(pt[1],6),'ptZ':round(pt[2],6),'isRepeated':False,'lineLoopMark':countLine,'surfaceMark':countSurface,'volumeMark':countVolume})
                #end for
            #end for
    ...
    return [ptDict,lineDict, lineLoopDict, surfDict, volDict]
...
def  isSameSurf(lineLoopIndexList1, lineLoopIndexList2):
