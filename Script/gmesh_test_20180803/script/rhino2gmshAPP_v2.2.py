...
def setPointSize(ptDict):
    ...
    for layer in layerList: #each sub layer
        if rs.IsLayerOn(layer):
            ...
            objs=rs.ObjectsByLayer(layer,False)# objs is a list of guids for point
            for obj in objs:
                pt=rs.coerce3dpoint(obj) #get the pt tuple with xyz by guid
                myPt=Point(pt[0],pt[1],pt[2])
                c.execute('update ptTable set meshSize=? where ptX=? and ptY=? and ptZ=? and isRepeated=?', (ptMeshSize,myPt.x,myPt.y,myPt.z,'False'))
                index=c.execute('select ptDictNo from ptTable where ptX=? and ptY=? and ptZ=? and isRepeated=?',(myPt.x,myPt.y,myPt.z,'False'))
                res=index.fetchall()
